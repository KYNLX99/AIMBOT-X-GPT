...
local function IsESPEnemy(player)
	-- ESP menampilkan SEMUA player selain diri sendiri.
	-- Berbeda dengan aimbot yang tetap memakai IsEnemy().
			...
			if Head then
				...
				if OnScreen and ScreenPosition.Z > 0 then
					...
					if ScreenDistance <= FOV then
						-- Prefer smaller screen distance; use world distance as a tie-breaker.
						local CameraDistance =
							(Camera.CFrame.Position - Head.Position).Magnitude

						local Score = ScreenDistance + (CameraDistance * 0.0001)

						if Score < ClosestScore and CanSeeTarget(player) then
							ClosestScore = Score
							Closest = player
						end
					end
...
local function AimAtHead(player, dt)
	...
	local Desired = CFrame.lookAt(CurrentPosition, Head.Position)

	-- Smoothness 1.0 = instant; lower values = more gradual.
	...
	local frameAlpha = 1 - math.exp(-strength * 18 * math.max(dt or (1 / 60), 1 / 240))
	Camera.CFrame = Camera.CFrame:Lerp(Desired, frameAlpha)

	-- Remove tiny residual error when almost centered.
...
RunService:BindToRenderStep(
	...
	function(dt)
		...
		-- Validate the current lock.
		if CurrentTarget and not IsValidTarget(CurrentTarget) then
			CurrentTarget = nil
		end

		-- Acquire a target if we do not have one.
		if not CurrentTarget then
			CurrentTarget = GetClosestTarget()
		elseif not StickyLock then
			-- Non-sticky mode reevaluates every frame.
			local better = GetClosestTarget()
		...
		-- Keep the lock while the enemy remains valid.
		-- With Wall Check enabled, the aim pauses behind cover rather than switching.
		if CanSeeTarget(CurrentTarget) then
			AimAtHead(CurrentTarget, dt)
		end

		-- If the target becomes invalid right after the aim step, reacquire immediately.
