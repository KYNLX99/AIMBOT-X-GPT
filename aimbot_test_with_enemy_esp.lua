...
local function IsESPEnemy(player)
	...
	-- Enemy-only: jika kedua pemain memiliki Team,
	-- tampilkan hanya pemain dari Team yang berbeda.
	if LocalPlayer.Team ~= nil and player.Team ~= nil then
		return player.Team ~= LocalPlayer.Team
	end

	-- Tanpa Team, jangan tampilkan agar tetap enemy-only.
	return false
end
...
local function CreateESP(player)
	...
	task.spawn(function()
		...
		if ESPObjects[player]
			and ESPObjects[player].Highlight == highlight then
			RemoveESP(player)
		end
	end)
end
...
RunService:BindToRenderStep(
	...
	function()
		...
		-- Kalau target mati, langsung cari target lain.
		if not CurrentTarget then
		...
		if CurrentTarget
			...
			-- Target tetap dipertahankan walaupun
			-- bergerak keluar dari FOV.
			if IsValidTarget(CurrentTarget) then

				-- Jika Wall Check aktif dan target
				-- berada di balik tembok, tunggu
				-- sampai target terlihat lagi.
		...
		-- Jika target baru saja mati ketika proses
		-- aim berlangsung, langsung cari penggantinya.
		if not CurrentTarget then
