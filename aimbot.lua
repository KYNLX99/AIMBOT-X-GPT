--[[
    ==============================================================
    LIYHUB AIMBOT + ESP + SILENT AIM — ULTIMATE EDITION
...
local Theme = {
    ...
    Silent           = Color3.fromRGB(236, 72, 153),  -- pink untuk silent FOV
}
...
local Config = {
    ...
    SilentAim       = false,
    SilentFov       = 200,
    SilentHitPart   = "Head",          -- Head / Torso / Random / Nearest
    SilentHitChance = 100,             -- 0-100
    SilentDrawFov   = true,
    SilentTeamCheck = true,
    SilentWallcheck = false,
    SilentKeywords  = "fire,shoot,attack,weapon,gun,bullet,projectile,ray,swing,sword,damage,hit,shot,melee,combat,castgun",
    ...
    EspVisibleColor = Color3.fromRGB(16, 185, 129),
...
pcall(function()
    if Drawing then DrawingLib = Drawing
    elseif getgenv and getgenv().Drawing then DrawingLib = getgenv().Drawing
    elseif syn and syn.Drawing then DrawingLib = syn.Drawing
    end
end)
...
local function updateDrawingESP(plr)
    ...
    local head = char:FindFirstChild("Head")
    if not hum or not hrp or not head or hum.Health <= 0 then hideAll(d); return end

    local camPos = Camera.CFrame.Position
    local dist = (hrp.Position - camPos).Magnitude
    if dist > Config.EspMaxDistance then hideAll(d); return end
    ...
    if not (headVis and hrpVis) then hideAll(d); return end
    ...
    else pcall(function() d.Name.Visible = false end) end
    ...
    else pcall(function() d.Distance.Visible = false end) end

    if Config.EspTracer then
        local vp = Camera.ViewportSize
        local fromX, fromY = vp.X / 2, (Config.EspTracerFrom == "Bottom" and vp.Y or vp.Y / 2)
        pcall(function()
    ...
    else pcall(function() d.HeadDot.Visible = false end) end

    if Config.EspHealth then
        local pct = math.clamp(hum.Health / math.max(hum.MaxHealth, 1), 0, 1)
        local hpX, hpW, hpY, hpH = boxX - 7, 3, boxY, boxH
        local healthColor = Color3.fromRGB(math.floor(255*(1-pct)), math.floor(255*pct), 60)
        pcall(function()
    ...
    if Config.EspSkeleton then
        ...
        local pts = {
            head = w2s(head),
            upper = w2s(char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")),
            lower = w2s(char:FindFirstChild("LowerTorso") or char:FindFirstChild("Torso")),
            lArm = w2s(char:FindFirstChild("LeftUpperArm") or char:FindFirstChild("Left Arm")),
            lHand = w2s(char:FindFirstChild("LeftLowerArm") or char:FindFirstChild("Left Arm")),
            rArm = w2s(char:FindFirstChild("RightUpperArm") or char:FindFirstChild("Right Arm")),
            rHand = w2s(char:FindFirstChild("RightLowerArm") or char:FindFirstChild("Right Arm")),
            lLeg = w2s(char:FindFirstChild("LeftUpperLeg") or char:FindFirstChild("Left Leg")),
            lFoot = w2s(char:FindFirstChild("LeftLowerLeg") or char:FindFirstChild("Left Leg")),
            rLeg = w2s(char:FindFirstChild("RightUpperLeg") or char:FindFirstChild("Right Leg")),
            rFoot = w2s(char:FindFirstChild("RightLowerLeg") or char:FindFirstChild("Right Leg")),
        }
...
local function renderESP()
    if not Config.EspEnabled then
        ...
        for _, data in pairs(billboardCache) do
...
-- FOV Circle (silent aim — warna pink, di luar)
...
SilentFovCircle.Size = UDim2.fromOffset(Config.SilentFov * 2, Config.SilentFov * 2)
SilentFovCircle.BackgroundTransparency = 1
SilentFovCircle.BorderSizePixel = 0
SilentFovCircle.ZIndex = 4
...
SilentFovStroke.Color = Theme.Silent
SilentFovStroke.Thickness = 1.2
SilentFovStroke.Transparency = 0.45
...
local FRAME_H = isSmall and math.clamp(vp.Y - 100, 380, 560) or 560
...
local function makeSectionTitle(order, text, color)
...
-- ==================== SILENT AIM — TARGET UTILS ====================
local function getSilentHitPart(char)
    if not char then return nil end
    if Config.SilentHitPart == "Head" then
        return char:FindFirstChild("Head")
    elseif Config.SilentHitPart == "Torso" then
        return char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
    elseif Config.SilentHitPart == "Random" then
        local parts = { "Head", "UpperTorso", "Torso", "HumanoidRootPart" }
        local avail = {}
        for _, n in ipairs(parts) do
            local p = char:FindFirstChild(n)
            if p then table.insert(avail, p) end
        end
...
local function isSameTeam(plr, check)
    if not check then return false end
    if not LocalPlayer.Team then return false end
    return plr.Team == LocalPlayer.Team
end

local function silentWallcheck(char, targetPart)
    if not Config.SilentWallcheck then return true end
    local origin = Camera.CFrame.Position
...
-- Silent Aim target: gunakan SilentFov (bukan FovRadius)
local function getSilentTarget()
    if not Config.SilentAim then return nil end
    -- Hit chance roll
    if Config.SilentHitChance < 100 then
        if math.random(1, 100) > Config.SilentHitChance then return nil end
    end
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            ...
            if hum and hum.Health > 0 and not isSameTeam(plr, Config.SilentTeamCheck) then
                local part = getSilentHitPart(plr.Character)
                if part then
                    local sp, onScreen = Camera:WorldToViewportPoint(part.Position)
                    if onScreen then
                        local dx, dy = sp.X - center.X, sp.Y - center.Y
                        if math.sqrt(dx*dx + dy*dy) <= Config.SilentFov then
                            if silentWallcheck(plr.Character, part) then
...
local hookedMetatable = nil

local function getKeywordsSet()
    local set = {}
    for kw in tostring(Config.SilentKeywords or ""):gmatch("[^,]+") do
        local trimmed = kw:match("^%s*(.-)%s*$"):lower()
        if trimmed ~= "" then set[trimmed] = true end
    end
    return set
end

local function matchKeyword(remoteName, set)
    if not remoteName or remoteName == "" then return false end
    local lower = remoteName:lower()
    for kw, _ in pairs(set) do
...
local function setupSilentAimHook()
    ...
    if not (getrawmetatable and setreadonly and getnamecallmethod and newcclosure) then
        warn("[LiyHub Silent Aim] Executor tidak mendukung metatable hook.")
        notify("⚠️ Silent Aim Unsupported",
            "Executor kamu tidak support metatable hook (getrawmetatable, dll).", 6)
        return
    end

    local ok = pcall(function()
        hookedMetatable = getrawmetatable(game)
        originalNamecall = hookedMetatable.__namecall
        setreadonly(hookedMetatable, false)

        hookedMetatable.__namecall = newcclosure(function(self, ...)
            local method = getnamecallmethod()
            local args = { ... }

            -- Cek apakah target valid untuk dimodifikasi
            if Config.SilentAim and Config.Enabled
               and (method == "FireServer" or method == "InvokeServer")
               and typeof(self) == "Instance"
               and (self:IsA("RemoteEvent") or self:IsA("RemoteFunction")) then

                local keywords = getKeywordsSet()
                if matchKeyword(self.Name, keywords) then
                    local targetPart = getSilentTarget()
                    if targetPart then
                        local newDir = (targetPart.Position - Camera.CFrame.Position).Unit
                        local newCF  = CFrame.lookAt(Camera.CFrame.Position, targetPart.Position)

                        for i, arg in ipairs(args) do
                            local t = typeof(arg)
                            if t == "Vector3" then
                                -- Asumsi arg ini adalah direction/origin
                                -- Jika magnitude > 1, kemungkinan origin (biarkan)
                                -- Jika magnitude <= 1, kemungkinan direction (ganti)
                                if arg.Magnitude <= 1.5 then
                                    args[i] = newDir
                                else
                                    -- Kemungkinan posisi origin — biarkan, atau ganti?
                                    -- Coba: kalau argumen berikutnya Vector3 juga, kita biarkan
                                end
        ...
        setreadonly(hookedMetatable, true)
        silentAimHookActive = true
    end)

    if not ok then
        silentAimHookActive = false
        notify("⚠️ Silent Aim Hook Failed", "Gagal hook __namecall. Cek output console.", 6)
    else
        notify("🎯 Silent Aim Hooked", "Hook __namecall aktif.", 4)
    end
end

local function removeSilentAimHook()
    if not silentAimHookActive then return end
    pcall(function()
        if hookedMetatable and originalNamecall then
            setreadonly(hookedMetatable, false)
            hookedMetatable.__namecall = originalNamecall
            setreadonly(hookedMetatable, true)
        end
    end)
    silentAimHookActive = false
...
-- SILENT AIM
makeSectionTitle(nextOrder(), "Silent Aim", Theme.Silent)
makeToggle(nextOrder(), "Silent Aim Master", Config.SilentAim, function(v)
    Config.SilentAim = v; saveConfig()
    if v then
        setupSilentAimHook()
    else
        removeSilentAimHook()
    ...
    notify(v and "🎯 Silent Aim ON" or "🚫 Silent Aim OFF",
        v and "Hook aktif, peluru diarahkan ke target." or "Hook dilepas.", 3)
end)
makeSlider(nextOrder(), "Silent FOV", 40, 600, Config.SilentFov, false, function(v)
    Config.SilentFov = v
    SilentFovCircle.Size = UDim2.fromOffset(v * 2, v * 2)
    saveConfig()
end)
makeSlider(nextOrder(), "Silent Hit Chance (%)", 0, 100, Config.SilentHitChance, false, function(v)
    Config.SilentHitChance = v; saveConfig()
end)
makeToggle(nextOrder(), "Silent Draw FOV", Config.SilentDrawFov, function(v) Config.SilentDrawFov = v; saveConfig() end)
makeToggle(nextOrder(), "Silent Team Check", Config.SilentTeamCheck, function(v) Config.SilentTeamCheck = v; saveConfig() end)
makeToggle(nextOrder(), "Silent Wallcheck", Config.SilentWallcheck, function(v) Config.SilentWallcheck = v; saveConfig() end)
makeDropdown(nextOrder(), "Silent Hit Part", { "Head", "Torso", "Random", "Nearest" },
    ({ Head = 1, Torso = 2, Random = 3, Nearest = 4 })[Config.SilentHitPart] or 1,
    function(v) Config.SilentHitPart = v; saveConfig() end)
makeActionButton(nextOrder(), "🔗  Apply Custom Keywords", Theme.Silent, function()
    if _G.__LiyHubEditKeywords then _G.__LiyHubEditKeywords() end
end)
...
makeToggle(nextOrder(), "ESP Master", Config.EspEnabled, function(v)
    Config.EspEnabled = v; saveConfig()
...
makeSlider(nextOrder(), "ESP Max Distance", 100, 5000, Config.EspMaxDistance, false, function(v)
    Config.EspMaxDistance = v; saveConfig()
end)
...
-- ==================== EDIT KEYWORDS (TextBox Prompt) ====================
_G.__LiyHubEditKeywords = function()
    -- Simple prompt via StarterGui:SetCore (jika didukung)
    -- Fallback: bikin textbox overlay kecil
    if guiParent:FindFirstChild("LiyHubKeywordsPrompt") then
        guiParent.LiyHubKeywordsPrompt:Destroy()
    end

    local prompt = Instance.new("ScreenGui")
    prompt.Name = "LiyHubKeywordsPrompt"
    prompt.ResetOnSpawn = false
    prompt.DisplayOrder = 10000
    prompt.Parent = guiParent
    ...
    titleLbl.Text = "SILENT AIM — CUSTOM KEYWORDS"
    titleLbl.TextColor3 = Theme.Silent
    ...
    hintLbl.Text = "Pisahkan dengan koma. Contoh: fire,shoot,attack,gun"
    hintLbl.TextColor3 = Theme.TextSecondary
    hintLbl.TextSize = 10
    ...
    box.Text = Config.SilentKeywords
    ...
    saveBtn.Size = UDim2.new(0.5, -15, 0, 32)
    saveBtn.Position = UDim2.new(0, 10, 1, -42)
    saveBtn.BackgroundColor3 = Theme.Silent
    saveBtn.Text = "Save"
    ...
    cancelBtn.Position = UDim2.new(0.5, 5, 1, -42)
    cancelBtn.BackgroundColor3 = Theme.SurfaceSecondary
    ...
    saveBtn.MouseButton1Click:Connect(function()
        Config.SilentKeywords = box.Text
        saveConfig()
        notify("🔗 Keywords Updated", "Custom keywords berhasil disimpan.", 3)
        prompt:Destroy()
    end)
...
local function hasLineOfSight(targetChar)
    local targetPart
    if Config.TargetPart == "Head" then targetPart = targetChar:FindFirstChild("Head")
    elseif Config.TargetPart == "UpperTorso" then targetPart = targetChar:FindFirstChild("UpperTorso")
    elseif Config.TargetPart == "Torso" then targetPart = targetChar:FindFirstChild("Torso")
    else targetPart = targetChar:FindFirstChild("Head") or targetChar:FindFirstChild("UpperTorso") end
...
local function getTargetPart(char)
    if not char then return nil end
    if Config.TargetPart == "Head" then return char:FindFirstChild("Head")
    elseif Config.TargetPart == "UpperTorso" then return char:FindFirstChild("UpperTorso")
    elseif Config.TargetPart == "Torso" then return char:FindFirstChild("Torso") end
    local best, bestD = nil, math.huge
    local camPos = Camera.CFrame.Position
...
local function updateVelocity(plr, part)
    ...
    if cache then
        ...
        if dt > 0.001 then cache.velocity = (part.Position - cache.pos) / dt end
...
local function getTargetInFov(exclude)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr ~= exclude and plr.Character then
            ...
            if hum and hum.Health > 0 and part and not isSameTeam(plr, Config.TeamCheck) then
                local sp, onScreen = Camera:WorldToViewportPoint(part.Position)
                if onScreen then
                    local dx, dy = sp.X - center.X, sp.Y - center.Y
...
local function isValidTarget(plr)
    ...
    if isSameTeam(plr, Config.TeamCheck) then return false end
    local sp, onScreen = Camera:WorldToViewportPoint(part.Position)
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton2 then
        if Config.AimMode == "Hold" then aimHeld = true
        elseif Config.AimMode == "Toggle" then aimToggled = not aimToggled end
    end
end)
        ...
        if stickyLocked and currentTarget then
            if not isValidTarget(currentTarget) then
                if Config.AutoSwitch then
                    ...
                    if newT then currentTarget = newT
                    else stickyLocked = false; currentTarget = nil end
                else
                    stickyLocked = false; currentTarget = nil
                end
            end
        end
        if not (stickyLocked and Config.StickyLock and currentTarget) then
            local found = getTargetInFov()
            if found then currentTarget = found; stickyLocked = true
            else currentTarget = nil; stickyLocked = false end
        end
        -- Jika Silent Aim aktif, JANGAN ubah CFrame kamera
        if currentTarget and stickyLocked and not Config.SilentAim then
            aimAt(currentTarget)
        end
        if Config.SilentAim and silentAimHookActive then
            StatusLbl.Text = "Status: SILENT → " .. (currentTarget and currentTarget.Name or "(waiting)")
            StatusLbl.TextColor3 = Theme.Silent
        elseif currentTarget and stickyLocked then
...
task.spawn(function()
    ...
    while task.wait(0.03) do
        ...
        SilentFovCircle.Visible = Config.SilentAim and Config.SilentDrawFov and MainFrame.Visible
...
-- ==================== INIT ====================
task.spawn(function()
    local loader = createLoadingOverlay("Initializing Aim + ESP + Silent")
    ...
    if loader then loader:Update(80) end

    -- Auto-enable silent aim hook jika config true
    if Config.SilentAim then
        task.spawn(setupSilentAimHook)
    end
    ...
    local espMode = HAS_DRAWING and "Drawing API" or "BillboardGui"
    local hookSupport = (getrawmetatable and newcclosure) and "Yes" or "No"
    notify("🎯 LiyHub Aimbot + ESP + Silent",
        string.format("Exec: %s • ESP: %s • Hook: %s", execName, espMode, hookSupport), 6)

    print("[LiyHub] Loaded on " .. execName .. " | ESP: " .. espMode .. " | Hook Support: " .. hookSupport)
end)
